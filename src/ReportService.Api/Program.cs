...
if (string.IsNullOrWhiteSpace(connectionString))
    throw new InvalidOperationException("ConnectionStrings:Default não configurada. Defina via variável de ambiente ou appsettings.");
...
app.UseExceptionHandler(a => a.Run(async ctx =>
{
    ...
    await ctx.Response.WriteAsJsonAsync(new ErrorResponse("INTERNAL_ERROR", "Erro interno.", []));
}));
...
{
    ctx.Items["CorrelationId"] = ctx.Request.Headers["X-Correlation-Id"].FirstOrDefault() ?? Guid.NewGuid().ToString();
    await next();
});
